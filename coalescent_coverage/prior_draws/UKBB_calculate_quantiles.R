...
AF_list              = do.call('rbind', lapply(list.files(path = "results/UKBB/", pattern = sprintf("%s_%s_AF_Region_%s_Chunk_1.RData",ref_pop,fst,regions),full.names = T), readRDS))

#Read in all chunks, per replicate (for each divergence time) for the LD
LD_file_list         = list.files(path = "results/UKBB/", pattern = sprintf("%s_%s_LD_Region_%s_Chunk_1.RData",ref_pop,fst,regions), full.names = T)
...
saveRDS(LD_results,sprintf("results/UKBB/%s_%s_LD_quantile_counts_region_%s.RData", ref_pop, fst, regions), version = 2)
saveRDS(AF_results,sprintf("results/UKBB/%s_%s_AF_quantile_counts_region_%s.RData", ref_pop, fst, regions), version = 2)
