...
#Read in GWAS panel
gwas_case_haplotypes     = t(as.matrix(read.table(sprintf("hapgen2/gwas_region_1.cases.haps",regions, header = T)))) #Read in the GWAS haplotypes (cases and control)
gwas_control_haplotypes  = t(as.matrix(read.table(sprintf("hapgen2/gwas_region_1.controls.haps",regions, header = T)))) #Read in the GWAS haplotypes (cases and control)
#Merge together the haplotypes
gwas_haplotypes = rbind(gwas_case_haplotypes,gwas_control_haplotypes)

#Read in Ref panel
ref_pop_A_haplotypes    = as.matrix(read.table(sprintf("msprime_data/joint_filtered/ref_pop_A_region_%s", regions), header = T))
ref_pop_B_haplotypes    = as.matrix(read.table(sprintf("msprime_data/joint_filtered/ref_pop_B_region_%s", regions), header = T))
...
gwas_ld_true_sumstats     = paste(sprintf("finemap/true_sumstats_region_%s.z", regions),
                                  sprintf("finemap/gwas_region_%s.ld", regions),
                                  sprintf("finemap/gwas_ld_true_sumstats_region_%s.snp", regions),
                                  sprintf("finemap/gwas_ld_true_sumstats_region_%s.config", regions),
                                  sprintf("finemap/gwas_ld_true_sumstats_region_%s.cred",regions),
                                  sprintf("finemap/gwas_ld_true_sumstats_region_%s.log",regions),
                                  10000,
                                  sep = ";")
...
ref_LD  = LD_Matrix(rbind(ref_pop_A_haplotypes[1:2000,],ref_pop_B_haplotypes[1:2000,]))
...
write.table(ref_LD, file = sprintf("finemap/gwas_region_%s.ld", regions), quote = F, col.names = F, row.names = F)
