...
for (i in 1:999) {
  ...
  target_pop = as.matrix(fread(sprintf("pop_split_msprime/target_1_replicate_%s_dt_%s_recomb_%s.csv",i,divergence_time,recombination_rate),header = T))
  
  #Get constants
  nhaps_target   = nrow(target_pop)
  nsnps_target   = ncol(target_pop)
  ...
  peripheral_list_names  = list.files(path = "pop_split_msprime", pattern = sprintf("peripheral_\\d+_replicate_%s_dt_%s_recomb_%s.csv",i,divergence_time,recombination_rate), full.names = T)
  ...
  #Get the AF of the SNPs across the peripheral populations
  snps_af_peripheral = apply(peripheral_array, c(3), colMeans)
  ...
  if (length(pops_removed) > (peripheral_pop-2)){
    results = list(-4,-4,-4,-4)
    saveRDS(results,sprintf("pop_split_results/results_replicate_%s_dt_%s_recomb_%s",i,divergence_time,recombination_rate))
    ...
    if (length(pops_removed) > 0) {
      #Calculate the geometric mean of the SNP in target population
      gm_snps            = exp(mean(log(colMeans(target_pop))))

      #Calculate the LD between the SNPs
      if (length(dim(peripheral_array[,,-pops_removed])) == 2) {
        print(dim(peripheral_array[,,-pops_removed]))
        ld_peripheral             = apply(peripheral_array[,,-pops_removed], c(2), cov.wt,method = "ML", cor = TRUE)
      }
      
      if (length(dim(peripheral_array[,,-pops_removed])) == 3) {
      ...
      #Get the variance and mean of the peripheral population LD values
      var_ld_peripheral  = var(ld_peripheral_values)
      mean_ld_peripheral = mean(ld_peripheral_values)
    }
    #Save results
