...
for i in range(1,ngwas_replicates):
  ...
  mutated_ts = msprime.sim_mutations(tree, rate=1e-6, model = "binary") 
  ...
  np.savetxt("pop_split/msprime/positions_gwas_%s_dt_%s" % (i,divergence_time), positions, newline=" ")
  ...
  ref.to_csv("pop_split/msprime/ref_%s_dt_%s.csv" % (i,divergence_time), index = False)
  #Output the GWAS population
  gwas = pd.DataFrame(haps[:ngwas_haps*2,])
  print(np.shape(gwas))
  gwas.to_csv("pop_split/msprime/gwas_%s_dt_%s.csv" % (i,divergence_time), index = False)
