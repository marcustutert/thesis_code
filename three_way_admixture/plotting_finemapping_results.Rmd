...
Inferred_LD       = rep("Infer LD",nregions)
...
fig <- fig %>%
  layout(
    title = "Admixture Finemapping Posterior Probability Accuracy across LD Panels",
    xaxis = list(
      title = "LD Panel"
    ),
    yaxis = list(
      title = "Weighted Posterior of Causal Variant in 95% Credible Set ",
      zeroline = F
    )
  )
...
fig <- fig %>% layout(yaxis = list(title = "Number of Regions",type = 'linear'), barmode = 'group', title = "Number of Causal Variants Assigned in 95% Credible Set Across Panels", xaxis = list(title = "Panel"))
...
nregions            = 19  #Number of regions
nSamples            = 100 #Number of samples we draw from the LD distribution

#True proportions of false positive across regions and bins
tp_GWAS_LD          = array(data = NA, dim = c(nSamples,nregions,10)) 
tp_EAS_LD           = array(data = NA, dim = c(nSamples,nregions,10))  
tp_EUR_LD           = array(data = NA, dim = c(nSamples,nregions,10))
tp_AFR_LD           = array(data = NA, dim = c(nSamples,nregions,10)) 
tp_1000G_LD         = array(data = NA, dim = c(nSamples,nregions,10)) 
tp_Ref_Admixed_LD   = array(data = NA, dim = c(nSamples,nregions,10))  
tp_Infer_LD         = array(data = NA, dim = c(nSamples,nregions,10))  
tp_Infer_Distr_LD   = array(data = NA, dim = c(nSamples,nregions,10))  

#Mean PIPs in each bin
mean_pip_GWAS_LD        = array(data = NA, dim = c(nSamples,nregions,10)) 
mean_pip_EAS_LD         = array(data = NA, dim = c(nSamples,nregions,10))
mean_pip_EUR_LD         = array(data = NA, dim = c(nSamples,nregions,10)) 
mean_pip_AFR_LD         = array(data = NA, dim = c(nSamples,nregions,10)) 
mean_pip_1000G_LD       = array(data = NA, dim = c(nSamples,nregions,10))
mean_pip_Ref_Admixed_LD = array(data = NA, dim = c(nSamples,nregions,10)) 
mean_Infer_LD           = array(data = NA, dim = c(nSamples,nregions,10)) 
mean_Infer_Distr_LD     = array(data = NA, dim = c(nSamples,nregions,10)) 


#Generate all the NON Distribrution Panels
for (i in 1:nregions) {   #Loop through regions
      ...
      for (k in 1:length(breaks_labels)) {
          ...
          tp_EUR_LD[j,i,k]       = (length(which(cut_bins_posterior$position %in% snp_causal)))/nrow(cut_bins_posterior)
          mean_pip_EUR_LD[j,i,k] = mean(cut_bins_posterior$prob)
   }
}

averaged_mean_pip_EUR_LD = rowMeans(mean_pip_EUR_LD,na.rm = T)
tp_GWAS_LD               = rowMeans(tp_EUR_LD,na.rm = T)
